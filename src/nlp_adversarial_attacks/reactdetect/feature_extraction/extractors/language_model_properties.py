...
def lm_proba_and_rank(
    ...
    if feature_list is not None:
        for i in range(len(regions)):
            ...
            feature_list += [
                f"lm_proba_quant{j}_region{i}" for j in range(len(quantiles))
            ]
            feature_list.append(f"lm_rank_mean_region{i}")
            feature_list.append(f"lm_rank_var_region{i}")
            feature_list += [
    ...
    for i, text in enumerate(dataloader):
        ...
        for start_frac, end_frac in regions:
            ...
            region_ids = lm_masked_tokenizer(
                region_text, truncation=True, return_tensors="pt"
            ...
            if len(region_ids) > max_length:
                ...
                for j in range(0, len(region_ids), max_length):
                    ...
                    segment_logits = lm_masked_model(
                        segment_ids.unsqueeze(dim=0), return_dict=True
                    ).logits[0]
            ...
            # compute all logits at the same time
                ...
                region_logits = lm_masked_model(
                    region_ids.unsqueeze(dim=0), return_dict=True
                ).logits[0]
            ...
            seq_proba = [
                probas[i][region_ids[i]].item() for i in range(len(region_ids))
            ]
...
def lm_perplexity(
    ...
    if hasattr(lm_causal_model.config, "n_positions"):
    ...
    for i, text in enumerate(dataloader):
        ...
        for start_frac, end_frac in regions:
            ...
            region_ids = lm_causal_tokenizer(
                region_text, return_tensors="pt", truncation=True
            )["input_ids"][0].to(device)
            ...
            for j in range(0, region_ids.size(0), stride):
                ...
                with torch.no_grad():
                    try:
                        ...
                        outputs = lm_causal_model(
                            input_ids.unsqueeze(0), labels=target_ids
                        )
