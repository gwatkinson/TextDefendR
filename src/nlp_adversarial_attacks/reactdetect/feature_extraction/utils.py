...
def save_extracted_samples(
    feature_dict: dict, text_list: list, df_: pd.DataFrame, path: str
):
    ...
    for i in range(len(text_list)):
        ...
        assert (
            df_.perturbed_text[i] == text_list[i]
        )  # always make sure text list and dataframe are aligned
        global_ndx = df_.global_ndx[
            i
        ]  # get "global" index in attack dataset for this sample
        sample_i["_sample_info"] = dict(
            df_.iloc[i]
        )  # also save row of attack dataset for sample
        ...
        if os.path.isfile(sample_i_out_dir):
            prev_sample_i = torch.load(
                sample_i_out_dir
            )  # load the previously saved features for sample i
            prev_sample_i.update(
                sample_i
            )  # update the previously saved sample's features with the new features
